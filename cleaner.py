...
# @decision DEC-HERMETIC-001
# @title Move all pipeline execution into main(); module import has zero side effects
# @status accepted
# @rationale
#   (a) Why the guard is needed: tests/test_likes.py and tests/test_should_keep.py
#       import cleaner to access pure helpers (should_keep, _resolve_likes_days,
#       Config, _parse_keep_tags).  When all execution code lived at module level,
#       every `import cleaner` immediately read config.json, called cli.login(),
#       paginated all records, ran the hydration loop, and called apply_writes.
#       This forced test authors to either stub the entire atproto + filesystem
#       surface (complex, fragile) or accept that running the test suite would
#       perform a real Bluesky login and a live cleanup pass — a test-hygiene defect.
#   (b) What is now module-level vs in main: module level exposes only imports,
#       constants (_MEDIA_EMBED_TYPES, _REGEX_ENTRY, _FLAG_MAP), and pure functions
#       and classes (Config, should_keep, paginated_list_records, _resolve_likes_days,
#       _parse_keep_tags).  Everything that touches the network, the filesystem, or
#       produces output (Config() construction, Client()/cli.login(), profile lookup,
#       paginated_list_records calls, hydration, the multi-pass delete loop, and
#       apply_writes) lives inside main() and only runs when the script is invoked
#       directly.
#   (c) Compatibility: `python cleaner.py` still works identically for end users —
#       the `if __name__ == "__main__": main()` guard at the bottom of the file
#       ensures main() is called exactly as before when the script is run directly.
def main():
    config = Config()
